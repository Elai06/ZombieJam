...
//#endif
        ...
        void InstallZenjectBinding(ZenjectBinding binding)
        {
            ...
            {
                Log.Warning("Found empty list of components on ZenjectBinding on object '{0}'", binding.name);
                return;
            }
            ...
            foreach (var component in binding.Components)
            {
                ...
                if (component == null)
                {
                    Log.Warning("Found null component in ZenjectBinding on object '{0}'", binding.name);
                    continue;
                }
